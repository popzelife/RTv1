/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   rt_bound_box.c                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: qfremeau <qfremeau@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/01/04 16:56:29 by qfremeau          #+#    #+#             */
/*   Updated: 2017/01/04 20:55:12 by qfremeau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rtv1.h"

float		f_min(float const a, float const b)
{
	return (a < b ? a : b);
}

float		f_max(float const a, float const b)
{
	return (a > b ? a : b);
}

t_bound_box	*new_bound_box(t_vec3 *min, t_vec3 *max)
{
	t_bound_box		*b;

	b = malloc(sizeof(t_bound_box));
	b->min = min;
	b->max = max;
	return (b);
}

BOOL		hit_bound_box(t_bound_box *box, const t_ray *ray, float t_min, \
	float t_max)
{
	float		t0;
	float		t1;
	int			i;

	i = 0;
	while (i < 3)
	{
		t0 = f_min((v3_access(box->min, i) - v3_access(ray->orig, i)) / \
			v3_access(ray->dir, i), (v3_access(box->max, i) \
			- v3_access(ray->orig, i)) / v3_access(ray->dir, i));
		t1 = f_max((v3_access(box->min, i) - v3_access(ray->orig, i)) / \
			v3_access(ray->dir, i), (v3_access(box->max, i) \
			- v3_access(ray->orig, i)) / v3_access(ray->dir, i));
		t_min = f_min(t0, t_min);
		t_max = f_min(t1, t_max);
		if (t_max <= t_min)
			return (FALSE);
		++i;
	}
	return (TRUE);
}

/*
  Moving object from time0 to time1
*/

BOOL		surrounding_box(const t_bound_box *box0, const t_bound_box *box1)
{
	t_vec3		*small;
	t_vec3		*big;

	small = v3_new_vec(f_min(box0->min->x, box1->min->x),
		f_min(box0->min->y, box1->min->y),
		f_min(box0->min->z, box1->min->z));
	big = v3_new_vec(f_max(box0->max->x, box1->max->x),
		f_max(box0->max->y, box1->max->y),
		f_max(box0->max->z, box1->max->z));
	return (new_bound_box(small, big));
}
